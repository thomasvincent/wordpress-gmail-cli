...
namespace WordPressGmailCli\SocialAuth\Configuration;
...
class Configuration
{
     ...
     * @param string $type Type to cast to ('bool', 'int', 'float', 'string', 'array').
     * @param mixed $default Default value if key doesn't exist.
    ...
    public function getProviderConfig(string $provider): array
    {
        ...
        if (!is_array($config) || empty($config)) {
            throw new ConfigException(sprintf('No configuration found for provider "%s"', $provider));
        }

        // Check if provider is enabled
        if (!($config['enabled'] ?? false)) {
            throw new ConfigException(sprintf('Provider "%s" is disabled', $provider));
        }
